...
# Was previously a hardcoded absolute path (/home/claude/mango-kgrag/data/...)
# that only existed on one machine. Resolved relative to this script's own
# location instead, so `python gen_rag_data.py` works from a fresh clone on
# anyone's machine, writing to <repo_root>/data/new_rag_data.jsonl regardless
# of where the repo happens to be checked out.
OUTPUT_PATH = Path(__file__).resolve().parent / "data" / "new_rag_data.jsonl"
